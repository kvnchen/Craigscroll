Cs.MainPage = Ext.extend(Ext.util.Observable, {
    ...
    */
    ...
    loading:false,
    location:null,
   // query: {location:{country:'USA',state:'USA-CA',city:'USA-SFO-SNF'},
	//		category: 'RVAC'
	//},
	
	query: {location:'USA-SFO-SNF',category: 'RVAC'},
	pageCount: 0,
    ...
    start: function() {
		...
		//this.config.searchBox.city.on("change",this.search.createDelegate(this));
		this.config.searchBox.about.on("click",this.aboutUs.createDelegate(this));
    ...
    onWinScroll: function() {
        ...
        if (this.loading) {//console.log("Currently loading.");
			return;} //let it load
	...
	aboutUs: function() {
		alert("Welcome to CraigScroll.org, a new, visual way to browse CraigsList, created by interns of Weaver Mobile USA Inc: " 
		+ '\n' + '\n' + "Interns: Kelvin Chen, Eric Liu, Alec Deng, Victor Zhang, Jonah Lin, Haochun"
		+ '\n\n' + "Powered by 3taps API");
	...
	loadCatMenu: function(num) {
		...
		if (config.searchBox.catGroupList[num] == 'All Categories') {
			...
			for (var i=0; i<config.searchBox.catListEle.length; i++) {
				...
				config.searchBox.catListEle[i].on("click",this.search.createDelegate(this));
			}
		}
	},
    ...
    search: function(e) {
        ...
        loc = config.searchBox.search.city; 	// get loc and cat from search bar
        cat = config.searchBox.search.category;
		
		/*
			Notes:
			lookup dictionaries are in utils.js
			could they instead be replaced with dictionary objects?

			CONCLUSION: assigning temp = this, and then changing temp.query does
			infact change the query, at least in this search() scope?
		*/
		
		temp.query = {
			location: locationDict(loc), //returns city 
			text: config.searchBox.searchEntry.dom.value,
			category: categoryDict(cat)
        };

		var query = temp.query;        // reverted to temp.query for sanity reasons
		console.log(query); 
