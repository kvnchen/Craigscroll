Cs.SearchBox = Ext.extend(Cs.Box, {
    //values
    size:null,
    loc:{country:'US',state:'California',city:'San Francisco'},
    config:null,
    search:{city:'San Francisco',text:'',category:'vacation properties'},
    mainArea:'DUMMY MAIN',
    countryList:['US'],
    ...
    display: function(location) {
		...
		this.about = secondBar.createChild({
			tag:'div',
			cls:'search about click',
			html:'About Us',
		});
        ...
        for (var i=0; i<states.length; i++) { //for each state
            ...
            for (var j=0; j<cities.length; j++) { //for each city
				...
				tempCity.on('click',this.config.mainPage.search.createDelegate(this.config.mainPage));
            }
        }
